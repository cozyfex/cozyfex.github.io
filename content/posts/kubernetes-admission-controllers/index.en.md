...
`NamespaceAutoProvision` is one of the mutating controller.  
{{</admonition>}}
...
`NamespaceExists` is one of the validating controller.  
{{</admonition>}}
...
If there's not exist `blue` namespace, return error.  
{{</admonition>}}
...
If there's not exist `blue` namespace, it's created by the `NamespaceAutoProvision` that is admission controller.  
{{</admonition>}}
